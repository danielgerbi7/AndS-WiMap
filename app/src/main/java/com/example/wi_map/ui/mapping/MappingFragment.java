...
public class MappingFragment extends Fragment implements OnMapReadyCallback, INetworkClickListener, MenuProvider, SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private final ActivityResultLauncher<String[]> requestPermissionLauncher = registerForActivityResult(new ActivityResultContracts.RequestMultiplePermissions(), results -> {
        boolean fine = Boolean.TRUE.equals(results.get(Manifest.permission.ACCESS_FINE_LOCATION));
        boolean coarse = Boolean.TRUE.equals(results.get(Manifest.permission.ACCESS_COARSE_LOCATION));
    ...
    private void doScan() {
        if (!checkCoarsePermission()) return;
        locationClient.getLastLocation().addOnSuccessListener(loc -> {
            ...
            if (autoConnect && !results.isEmpty()) {
                ...
                boolean isOpen = !strongest.capabilities.contains("WEP") && !strongest.capabilities.contains("WPA");
            ...
            if (scanIntervalMs > 0) scanHandler.postDelayed(scanRunnable, scanIntervalMs);
        });
    }
    ...
    private WifiEntry buildEntry(ScanResult sr) {
        ...
        return new WifiEntry(sr.SSID, sr.level, dist, sr.BSSID, lastKnownLatLng.latitude, lastKnownLatLng.longitude);
    }

    private float calculateDistance(ScanResult sr) {
        LatLng fp = fingerprintMap.get(sr.BSSID);
        if (fp == null || lastKnownLatLng == null) return 0f;
        float[] d = new float[1];
        Location.distanceBetween(lastKnownLatLng.latitude, lastKnownLatLng.longitude, fp.latitude, fp.longitude, d);
        return d[0];
    }

    private void notifyIfNeeded(ScanResult sr) {
        if (!notifyThreshold || sr.level < thresholdDbm) return;
        sendNotification("Strong network: " + sr.SSID, "Signal " + sr.level + " dBm");
        Toast.makeText(requireContext(), "Threshold passed on " + sr.SSID + " (" + sr.level + " dBm)", Toast.LENGTH_SHORT).show();
    }
    ...
    private void updateMapMarkers() {
        ...
        mMap.addMarker(new MarkerOptions().position(lastKnownLatLng).title("You").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE)));


        for (WifiEntry entry : networks) {
            LatLng pos = fingerprintMap.get(entry.bssid);
            if (pos == null) continue;

            Marker marker = mMap.addMarker(new MarkerOptions().position(pos).title(entry.ssid).snippet(entry.level + " dBm · " + Math.round(entry.distanceM) + " " + (distanceUnit.equals("ft") ? "ft" : "m")));
    ...
    public void onNetworkClick(WifiEntry entry) {
        ...
        for (ScanResult candidate : lastScanResults) {
            if (candidate.BSSID.equals(entry.bssid)) {
                sr = candidate;
                break;
            }
        }
        if (sr == null) {
            Toast.makeText(requireContext(), "Network not found in last scan", Toast.LENGTH_SHORT).show();
            return;
        }
        boolean isOpen = !sr.capabilities.contains("WEP") && !sr.capabilities.contains("WPA");
        if (!isOpen) {
            Toast.makeText(requireContext(), "This network is secured — can’t connect automatically.", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void connectToOpenNetworkLegacy(ScanResult sr) {
        ...
        if (netId != -1) {
            ...
            Toast.makeText(requireContext(), "Failed to add network " + ssid, Toast.LENGTH_SHORT).show();
        }
    }

    @androidx.annotation.RequiresApi(29)
    private void connectToOpenNetworkQ(ScanResult sr) {
        WifiNetworkSpecifier spec = new WifiNetworkSpecifier.Builder().setSsid(sr.SSID).build();

        NetworkRequest req = new NetworkRequest.Builder().addTransportType(NetworkCapabilities.TRANSPORT_WIFI).setNetworkSpecifier(spec).build();
        ...
        cm.requestNetwork(req, new ConnectivityManager.NetworkCallback() {
            ...
            public void onAvailable(@NonNull Network network) {
                ...
                requireActivity().runOnUiThread(() -> Toast.makeText(requireContext(), "Connected to " + sr.SSID, Toast.LENGTH_SHORT).show());
            }

            @Override
            public void onUnavailable() {
                requireActivity().runOnUiThread(() -> Toast.makeText(requireContext(), "Failed to connect to " + sr.SSID, Toast.LENGTH_SHORT).show());
            }
        });
    }
    ...
    public void onMapReady(@NonNull GoogleMap googleMap) {
        ...
        boolean hasFine = ActivityCompat.checkSelfPermission(requireContext(), Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED;
        boolean hasCoarse = ActivityCompat.checkSelfPermission(requireContext(), Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED;
    ...
    private void sendNotification(String title, String text) {
        NotificationCompat.Builder builder = new NotificationCompat.Builder(requireContext(), CHANNEL_ID).setSmallIcon(R.drawable.wifi).setContentTitle(title).setContentText(text).setAutoCancel(true);
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
        switch (Objects.requireNonNull(key)) {
            ...
            case "pref_notify_level":
                ...
                doScan();
                Toast.makeText(requireContext(), "Notify threshold set to " + thresholdDbm + " dBm", Toast.LENGTH_SHORT).show();
                break;
